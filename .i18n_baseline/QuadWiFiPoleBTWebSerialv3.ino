...
#define NEOPIX_PIN 48  // pin 18 built-in NeoPixel AND light bar
...
void processPart(String part, int partIndex) {
  ...
  switch (partIndex) {
    ...
    case 8:
      ...
      break;
    default:
      if (partIndex >= 7 && commandCount < MAX_HOLDCOMMANDS) {
...
void doManualFlightUpdate() {
  ...
  if (packet_counter % 20 == 0 && connected) {
    ...
    // if (packet_counter % 200 == 0) {
        Serial.println("Pitch | Roll | Yaw | Throttle | Alt (m)");
        Serial.println("----------------------------------------");
    // }
    Serial.printf("%5d | %4d | %3d | %8d | %7.2f\n", 
                map(forward_back, 0, 255, 0, 100), map(left_right, 0, 255, 0, 100),
